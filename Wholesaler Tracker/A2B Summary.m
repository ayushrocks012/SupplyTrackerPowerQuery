let
    ...
    DemandFolder =
        Folder{[Name="General"]}[Content]
              ...
              {[Name="Daily Tracker Files"]}[Content],

    // === Filter for booking_overview CSV files ===
    FilteredFiles = Table.SelectRows(
        DemandFolder,
        each Text.StartsWith([Name], "booking_overview") and Text.EndsWith([Name], ".csv")
    ),
    ...
    SplitColumn = Table.SplitColumn(
        ...
        {"Breda Tour", "Departure"}
    ),
    RenamedColumns = Table.RenameColumns(
        SplitColumn,
        {{"Status", "Sub status"}, {"Delivery Date", "Expected Delivery Date"}}
    ),

    WithStatus = Table.AddColumn(
        RenamedColumns, "Status",
        each
    ...
    RemovedOtherColumns = Table.SelectColumns(
        AddedCustom,
        {"Booking ID", "Your Reference", "Collection Date", "Expected Delivery Date", "Sailing Date", "Breda Tour", "Departure", "Status", "Location", "Custom Cleared Status"}
    ),
